-- Create table to store challenge analysis results (Supabase compatible)
...
CREATE INDEX idx_challenge_analysis_challenge_id ON challenge_analysis(challenge_id);
CREATE INDEX idx_challenge_analysis_method ON challenge_analysis(analysis_method);
CREATE INDEX idx_challenge_analysis_completion ON challenge_analysis(completion_percentage DESC);
CREATE INDEX idx_challenge_analysis_created_at ON challenge_analysis(created_at DESC);
CREATE INDEX idx_challenge_analysis_can_complete ON challenge_analysis(can_complete);

-- Create a view for easy analysis comparison
CREATE VIEW challenge_analysis_summary AS
SELECT 
    challenge_id,
    challenge_title,
    analysis_method,
